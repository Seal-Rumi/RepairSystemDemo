...
function generateTable(data) {
  ...
  if (data) {
    for (var i = 0; i < data.length; i++) {
      ...
      assignTimeCell.innerHTML = infoArr[5];

      var deadlineCell = row.insertCell();
      deadlineCell.innerHTML = infoArr[6];
      ...
      reportBtn.classList.add("funcBtn", "reportBtn");
      reportBtn.innerHTML = `<i class="fas fa-comments"></i>回報`;
      // btnContainer.appendChild(reportBtn);

      var finishBtn = document.createElement("div");
      finishBtn.classList.add("funcBtn", "finishBtn");
      finishBtn.innerHTML = `<i class="fas fa-check"></i>完工`;

      var noBtn = document.createElement("div");
      noBtn.classList.add("funcBtn", "noBtn");
      noBtn.innerHTML = `完工`;
      // 將按鈕加入 span 容器
